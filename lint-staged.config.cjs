module.exports = {
  '*.{js,mjs,jsx,ts,tsx,yaml,yml,json,md,css,cjs}': (filenames) => {
    ...
    const cssFilenames = filenames
      .filter((name) => name.endsWith(`.css`))
      .map((name) => `"${name}"`)
      .join(' ');
    const stylelintCommand =
      cssFilenames.length > 0
        ? `npx stylelint --fix ${cssFilenames}`
        : `echo "stylelint skipped: no css files to lint"`;
